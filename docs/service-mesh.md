...
### 数据平面转发，控制平面分发意图 {#data-plane-forwards-control-plane-distributes-intent}
...
### 流量如何进入代理 {#how-traffic-enters-the-proxy}
...
### 身份先于加密 {#identity-before-encryption}
...
### 超时、重试、熔断与预算 {#timeouts-retries-circuit-breaking-and-budgets}
...
### 七层路由与渐进发布 {#layer-7-routing-and-progressive-delivery}
...
### 遥测来自代理视角 {#telemetry-from-the-proxy-perspective}
...
## 最小实践：运行本地 Envoy 代理 {#minimal-practice-run-a-local-envoy-proxy}

本实践让 Envoy 在 `127.0.0.1:10000` 接收 HTTP 请求，并转发给同一 Docker 网络中的最小后端。所有宿主机端口都绑定回环地址，不开启管理接口的公网访问。

### 1. 创建隔离网络与配置 {#create-isolated-network-and-configuration}
...
### 2. 启动后端与代理 {#start-backend-and-proxy}
...
### 3. 验证数据路径与状态 {#verify-data-path-and-status}
...
### 先建立采用基线 {#establish-an-adoption-baseline-first}
...
### 身份与授权 {#identity-and-authorization}
...
### 超时与重试治理 {#timeout-and-retry-governance}
...
### 可观测性与成本 {#observability-and-cost}
...
### 升级与故障域 {#upgrades-and-failure-domains}
...
### 入口、出口和旁路 {#ingress-egress-and-bypass}
...
### 认为 mTLS 等于零信任 {#equating-mtls-with-zero-trust}

mTLS 提供身份和加密，但若任何身份都可调用任何服务，仍缺少最小权限授权和持续验证。

### 对所有错误开启多次重试 {#retrying-all-errors-multiple-times}

重试会放大过载，也可能重复扣款等非幂等操作。应限制可重试状态、次数、总预算并加入退避抖动。

### 同时在应用、网关和网格重试 {#retrying-in-app-gateway-and-mesh}

多层重试相乘，使真实请求量失控。每条链必须指定重试责任层，并在指标中区分尝试与原始请求。

### 只看控制平面健康 {#only-checking-control-plane-health}

控制平面 Ready 不代表代理配置已接受、证书有效或业务链路成功。要同时监控配置分发、数据平面和端到端探测。

### 把代理遥测当作完整业务观测 {#treating-proxy-telemetry-as-complete-business-observability}

代理不知道订单是否正确完成，也看不到应用内部队列。应用指标、结构化日志和追踪埋点仍然必需。

### 一次性全量注入 {#injecting-everything-at-once}

协议不兼容、启动时序、资源不足或默认策略错误会同时影响大量服务。应按调用图和风险分批迁移。

### 直接暴露管理接口 {#exposing-administration-interfaces-directly}
